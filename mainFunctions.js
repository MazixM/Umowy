  ...
  var logData = function (text) {
    ...
    if (log) {
      ...
      localStorage.setItem(
        "log",
        JSON.stringify({ time: ConvertTimeToString(time), log: text })
      );
    }
  };
...
function createTableFromLocalvalues(divId) {
  var firstRow =
    '<tr><td colspan="5"><b>Aktualne umowy</b></td></tr>' +
  ...
  jQuery.each(records, function (_i, value) {
    if (value.dateTo <= today.getTime()) {
      color = "red";
    } else if (value.dateTo - today.getTime() > 1209600000) {
      // 14 days // 604800000) // 7 days
      color = "green";
    } else {
      color = "yellow";
    }
    var currentRow =
      ...
      value.name +
      "</td><td>" +
      ConvertTimeToString(value.dateFrom) +
      "</td><td>" +
      ConvertTimeToString(value.dateTo) +
      "</td><td><a href='" +
      value.urlHoursMonth +
      "' target='_blank'>" +
      value.hoursMonth +
  ...
  document.getElementById(divId).innerHTML =
    firstRow + actualContractRow + expiredRow + expiredContractRow;
}
...
function updateLocalRecordsTime() {
  ...
  jQuery.each(records, function (_i, value) {
    value.hoursMonth = getTimeByUrlOrReturnDefault(
      value.urlHoursMonth,
      value.hoursMonth
    );
    value.hoursYear = getTimeByUrlOrReturnDefault(
      value.urlHoursYear,
      value.hoursYear
    );
  });
